...
package com.eibus.applicationconnector.sap.xsd;
...
public class XSDDumper extends XMLBase
{
    ...
    private String m_sDefaultPrefix = "xsd";
	...
	public Element convert(XSDSchema xsdschema)
	{
		...
		if(doc != null)
		{
			...
			el = createElementNS(doc, ns_xsd, "schema");
	...
	private void xsddump_element(XSDElement xsdelement, Document doc, Element parent)
	{
        ...
        //pgussow: support for the xsd:any
        String sTagValue = "element";
        if (xsdelement.getElementType() == XSDElement.TYPE_ANY)
        {
            sTagValue = "any";
        }
        
		Element xsd_element = createElementNS(doc, ns_xsd, sTagValue);
	...
	private Element createElementNS(Document dDoc, String sNamespace, String sTagname)
    {
        Element eReturn = dDoc.createElementNS(sNamespace, sTagname);
        
        if (m_sDefaultPrefix != null && m_sDefaultPrefix.length() > 0 && XMLConstants.W3C_XML_SCHEMA_NS_URI.equals(sNamespace))
        {
            eReturn.setPrefix(m_sDefaultPrefix);
        }
	...
	private void xsddump_choice(XSDChoice xsdchoice, Document doc, Element parent)
	{
		...
		Element xsd_choice = createElementNS(doc, ns_xsd, "choice");
	...
	private void xsddump_complextype(XSDType xsdtype, Document doc, Element parent)
	{
		...
		Element xsd_complextype = createElementNS(doc, ns_xsd, "complexType");
		...
		if(basetype != null)
		{
			...
			{
				xsd_simplecontent = createElementNS(doc, ns_xsd, "complexContent");
			}
			else
			{
				xsd_simplecontent = createElementNS(doc, ns_xsd, "simpleContent");
			}
			...
			{
				xsd_extension = createElementNS(doc, ns_xsd, "restriction");
			}
			else
			{
				xsd_extension = createElementNS(doc, ns_xsd, "extension");
			}
		...
		if(xsdseq != null)
		{
			Element xsd_sequence = createElementNS(doc, ns_xsd, "sequence");
	...
	private void xsddump_simpletype(XSDType xsdtype, Document doc, Element parent)
	{
		...
		Element xsd_st = createElementNS(doc, ns_xsd, "simpleType");
		...
		{
			Element xsd_res = createElementNS(doc, ns_xsd, "list");
		...
		if(xsdtype.getType() == XSDType.TYPE_UNION)
		{
			...
			Element xsd_res = createElementNS(doc, ns_xsd, "union");
		...
		if(xsdres != null)
		{
			...
			Element xsd_res = createElementNS(doc, ns_xsd, "restriction");
			xsd_st.appendChild(xsd_res);
			...
			{
				Element xsd_in = createElementNS(doc, ns_xsd, "minInclusive");
				xsd_in.setAttribute("value", xsdres.getMinInclusive());
				xsd_res.appendChild(xsd_in);
			}
			if(res.get(XSDRestriction.RESTRICTION_MAX_INCLUSIVE))
			{
				Element xsd_in = createElementNS(doc, ns_xsd, "maxInclusive");
				xsd_in.setAttribute("value", xsdres.getMaxInclusive());
				xsd_res.appendChild(xsd_in);
			}
			if(res.get(XSDRestriction.RESTRICTION_MIN_EXCLUSIVE))
			{
				Element xsd_ex = createElementNS(doc, ns_xsd, "minExclusive");
				xsd_ex.setAttribute("value", xsdres.getMaxExclusive());
				xsd_res.appendChild(xsd_ex);
			}
			if(res.get(XSDRestriction.RESTRICTION_MAX_EXCLUSIVE))
			{
				Element xsd_ex = createElementNS(doc, ns_xsd, "maxExclusive");
				xsd_ex.setAttribute("value", xsdres.getMaxExclusive());
				xsd_res.appendChild(xsd_ex);
			}
			if(res.get(XSDRestriction.RESTRICTION_TOTAL_DIGITS))
			{
				Element xsd_td = createElementNS(doc, ns_xsd, "totalDigits");
				xsd_td.setAttribute("value", xsdres.getTotalDigits());
				xsd_res.appendChild(xsd_td);
			}
			if(res.get(XSDRestriction.RESTRICTION_FRACTION_DIGITS))
			{
				Element xsd_fd = createElementNS(doc, ns_xsd, "fractionDigits");
				xsd_fd.setAttribute("value", xsdres.getFractionDigits());
				xsd_res.appendChild(xsd_fd);
			}
			if(res.get(XSDRestriction.RESTRICTION_PATTERN))
			{
				Element xsd_pattern = createElementNS(doc, ns_xsd, "pattern");
			...
			{
				Element xsd_length = createElementNS(doc, ns_xsd, "length");
				xsd_length.setAttribute("value", xsdres.getLength());
				xsd_res.appendChild(xsd_length);
			}
			if(res.get(XSDRestriction.RESTRICTION_MIN_LENGTH))
			{
				Element xsd_ml = createElementNS(doc, ns_xsd, "minLength");
				xsd_ml.setAttribute("value", xsdres.getMinLength());
				xsd_res.appendChild(xsd_ml);
			}
			if(res.get(XSDRestriction.RESTRICTION_MAX_LENGTH))
			{
				Element xsd_ml = createElementNS(doc, ns_xsd, "maxLength");
				xsd_ml.setAttribute("value", xsdres.getMaxLength());
				xsd_res.appendChild(xsd_ml);
			}
			if(res.get(XSDRestriction.RESTRICTION_WHITE_SPACE))
			{
				Element xsd_ws = createElementNS(doc, ns_xsd, "whiteSpace");
				xsd_ws.setAttribute("value", xsdres.getWhiteSpace());
			...
			if(res.get(XSDRestriction.RESTRICTION_ENUMERATION))
			{
				...
				for(int i = 0; i < enumerations.size(); i++)
				{
					...
					Element xsd_e = createElementNS(doc, ns_xsd, "enumeration");
	...
	private void xsddump_attribute(XSDAttribute xsdatt, Document doc, Element parent)
	{
		...
		Element xsd_att = createElementNS(doc, ns_xsd, "attribute");
	...
	private void xsddump_attributegroup(XSDSequence xsdgroup, Document doc, Element parent)
	{
		...
		Element xsd_ag = createElementNS(doc, ns_xsd, "attributeGroup");
	...
	private void xsddump_elementgroup(XSDSequence xsdgroup, Document doc, Element parent)
	{
		...
		Element xsd_eg = createElementNS(doc, ns_xsd, "group");
