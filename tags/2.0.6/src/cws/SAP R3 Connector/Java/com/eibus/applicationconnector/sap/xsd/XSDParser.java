...
public class XSDParser extends XMLBase
{
	...
	private ArrayList<Object> schemas = null;
	// List of pre-loaded schemas (XML Element) and URLs (String)
	private ArrayList<Element> preloadschemas = null;
	private ArrayList<String> preloadfiles = null;
	// Namespace defaults
	...
	public XSDParser()
	{
		...
		this.preloadfiles = new ArrayList<String>();
		this.preloadschemas = new ArrayList<Element>();
	}
	...
	private boolean loadIncludes(XSDSchema xsdschema)
	{
		...
		ArrayList<Element> newschemas = new ArrayList<Element>();
		...
		if(newschemas.size() > 0)
		{
			...
			{
				Element schema = newschemas.get(i);
	...
	{
		XSDSchema xsdschema = new XSDSchema();
		ArrayList<String> schemanamespaces = new ArrayList<String>();
		...
		{
			String mtns = schemanamespaces.get(i);
			if(mtns != null)
			{
	...
	public XSDSchema parseSchemaElement(Element schema, int level)
	{
		...
		if(this.preloadfiles.size() > 0)
		{
			...
			{
				String xsdfile = this.preloadfiles.get(i);
	...
	private void reset()
	{
		...
		{
			XSDSchema xsdschema = (XSDSchema)this.preloadschemas.get(i);
	...
	private XSDAnnotation parseXSDAnnotation(Element el)
	{
		...
		String urlstr = el.getAttribute("source");
		if(urlstr.length() > 0)
		{
		...
		for(int i = 0; i < list.getLength(); i++)
		{
			...
			{
				xsdann.setDocumentation(textvalue(node));
			}
	...
	private ArrayList<Element> elementlist(Element parent, String filter)
	{
		if(parent == null)
		{
			return null;
	...
	private XSDType parseXSDSimpleType(Element simpletype)
	{
		...
		ArrayList<Element> list = elementlist(simpletype, null);
		...
		for(int i = 0; i < list.size(); i++)
		{
			...
			if(name.equals("restriction"))
			{
				...
				{
					ArrayList<Element> list2 = elementlist((Element)node, "simpleType");
					if(list2.size() == 1)
					{
